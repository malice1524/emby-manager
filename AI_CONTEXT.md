...
| 仪表盘 | `/api/dashboard/item/{id}` | GET | 媒体详情（含 TMDB/IMDb/演员） |
| 仪表盘 | `/api/dashboard/item/{id}` | DELETE | 删除媒体（需 EMBY_ADMIN_PW） |
| 仪表盘 | `/api/dashboard/images/{id}` | GET | 图片代理（item/user） |
| 用户 | `/api/users` | GET | 用户列表 |
| 用户 | `/api/users` | POST | 创建用户 |
| 用户 | `/api/users/{id}` | DELETE | 删除用户 |
| 用户 | `/api/users/{id}/password` | PUT | 修改密码 |
| 用户 | `/api/users/{id}/policy` | PUT | 修改策略（启用/禁用） |
| 媒体库 | `/api/libraries` | GET | 媒体库列表（含各类型数量统计） |
| 媒体库 | `/api/libraries/{id}/items` | GET | 媒体库内容（分页+搜索+类型过滤） |
| 系统 | `/api/health` | GET | 健康检查 |

所有端点返回 JSON，`Content-Type: application/json`。数量接口使用 `/emby/Items?ParentId=xxx&Recursive=true&Limit=0&IncludeItemTypes=Movie/Series/Episode` 分别查询，避免 `/Items/Counts` 接口忽略 ParentId 的问题。
...
### Libraries 组件
```javascript
data: {libraries, libSearch, searchResults, curLib, items, total, page, loadingMore, showItems, showDetail, libDetailItem, showLibDel}
// 搜索: libSearch → onGlobalSearch() → fetchJSON('/dashboard/recent?search=...')
// 详情弹窗: openLibItem() → fetchJSON('/dashboard/item/{id}')
// 删除: deleteLibItem() → DELETE /dashboard/item/{id}
```
...
### 文件路径
- **后端代码**: `backend/app/`（**重要**: 根目录 `app/` 是旧副本，修改后端时必须同步更新两处）
...
### Emby API 注意事项
- `/Items/Counts` 接口**忽略 ParentId 参数**，始终返回全局统计数据
- 获取每个媒体库的真实数量应使用 `/emby/Items?ParentId={id}&Recursive=true&Limit=0&IncludeItemTypes={type}` 并通过 `TotalRecordCount` 获取

## 后续开发注意事项
1. 修改 `libraries.py` 确认在 `backend/app/routers/` 下，同时同步 `app/routers/`
