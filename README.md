...
- 🗂️ **NFO 自动化** — 可视化填写 `tvshow.nfo`、抓取 PornHub 主视频中文标签写入 `tvshow.nfo`、批量抓取 PornHub 发布时间写入每集 `.nfo`、上传 `poster/fanart/logo`、上传剧集图并自动重命名、按 `.strm` 批量生成基础每集 `.nfo`；执行自动化后刷新当前 Emby 演员目录，单独保存/上传后可点手动刷新
